...
public class LookupService : ILookupService
{
    ...
    public async Task<LookupBundleResponseDto> GetBundleAsync(CancellationToken ct = default)
    {
        ...
        return new LookupBundleResponseDto
        {
            ...
            Shops = await _db.Shops.Where(x => x.TenantId == tenantId && !x.IsDeleted).OrderBy(x => x.Name).Select(x => new ShopLookupResponseDto { Id = x.Id, Name = x.Name, Code = x.Code }).ToListAsync(ct),
